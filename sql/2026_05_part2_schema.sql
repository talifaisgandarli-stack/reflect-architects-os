...
-- archived tasks: keep archived_at but map status → tamamlandı
...
-- Pipeline stage: ensure 'pipeline_stage' column exists (may already be 'stage')
alter table clients add column if not exists pipeline_stage text default 'Lead';

-- ── 5. activity_log ──────────────────────────────────────
create table if not exists activity_log (
  id uuid primary key default gen_random_uuid(),
  entity_type text not null,  -- 'task'|'project'|'client'|'income'|'expense'
...
create table if not exists project_documents (
  ...
  category text not null,  -- contract|price_protocol|invoice|act|email|letter|outsource_act
  title text not null,
  external_link text,
  internal_file_url text,
  source text default 'drive_link',  -- drive_link|auto_generated|email_capture
...
create table if not exists templates (
  id uuid primary key default gen_random_uuid(),
  category text not null,       -- letter|invoice|act|proposal|survey
  subcategory text,
  name text not null,
  language text default 'az',
  body_html text,
  variables jsonb default '[]', -- [{key, label, required}]
...
-- ── 14. Verification queries (run after migration) ───────
select
...
-- ── 15. Row count parity check ───────────────────────────
do $$
declare
  v_tasks_total int;
  v_valid_statuses int;
begin
  select count(*) into v_tasks_total from tasks;
  select count(*) into v_valid_statuses from tasks
  where status in ('İdeyalar','başlanmayıb','İcrada','Yoxlamada','Ekspertizada','Tamamlandı','Cancelled');
  if v_tasks_total = v_valid_statuses then
    raise notice 'PARITY CHECK PASSED — all % tasks have valid statuses', v_tasks_total;
  else
    raise warning 'PARITY CHECK FAILED — % tasks total but only % have valid statuses',
      v_tasks_total, v_valid_statuses;
  end if;
end $$;
