...
function NotifIcon({ type }) {
  if (type === 'success') return <IconCircleCheck size={14} className="text-green-500 flex-shrink-0 mt-0.5" />
  if (type === 'error')   return <IconAlertCircle  size={14} className="text-red-500   flex-shrink-0 mt-0.5" />
  if (type === 'warning') return <IconAlertCircle  size={14} className="text-yellow-500 flex-shrink-0 mt-0.5" />
  return                         <IconInfoCircle   size={14} className="text-blue-500  flex-shrink-0 mt-0.5" />
}
...
function NotifPanel({ notifs, onRead, onReadAll, onClose, navigate }) {
  ...
  return (
    <div
      className="absolute top-full right-0 mt-2 w-80 bg-white rounded-2xl border border-[#e8e8e4] z-50 overflow-hidden"
      ...
      <div className="max-h-96 overflow-y-auto">
        {notifs.length === 0 ? (
          <div className="py-12 text-center">
            <IconBell size={28} className="text-[#e8e8e4] mx-auto mb-2" />
          ...
          notifs.map(n => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-xs leading-snug ${!n.read ? 'font-semibold text-[#0f172a]' : 'font-medium text-[#555]'}`}>
                  {n.title}
                </div>
                {n.body && (
...
export default function MainLayout() {
  ...
  const loadNotifs = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('loadNotifs error:', error.message)
      } else {
        setNotifs(data || [])
      }
    } catch (e) {
      console.error('loadNotifs exception:', e.message)
    }
    setLoading(false)
  }, [user?.id])
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#fafaf8]">
        ...
        <div className="h-11 bg-white border-b border-[#e8e8e4] flex items-center px-3 lg:px-5 gap-3 flex-shrink-0">
          ...
          <div ref={bellRef} className="relative">
            ...
            >
              <IconBell size={16} />
              {unreadCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 bg-red-500 rounded-full flex items-center justify-center text-[8px] font-black text-white px-1 leading-none">
      ...
      {searchOpen && (
        <div
          className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4"
          onClick={() => setSearchOpen(false)}
        >
          <div
            className="w-full max-w-lg bg-white rounded-xl shadow-2xl overflow-hidden fade-in"
