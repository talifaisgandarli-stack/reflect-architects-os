...
import { IconPlus, IconEdit, IconTrash, IconArrowsExchange, IconAlertTriangle, IconCheck } from '@tabler/icons-react'
...
function TransferForm({ open, onClose, onSave, transfer, projects }) {
  const [form, setForm] = useState({
    from_project_id: '', to_project_id: '', amount: '',
    transfer_date: new Date().toISOString().split('T')[0],
    reason: '', return_deadline: '', status: 'open', notes: ''
  })
  ...
  useEffect(() => {
    if (transfer) {
      setForm({
        ...
        return_deadline: transfer.return_deadline || '',
        status: transfer.status || 'open',
        notes: transfer.notes || ''
      })
    } else {
      setForm({ from_project_id: '', to_project_id: '', amount: '', transfer_date: new Date().toISOString().split('T')[0], reason: '', return_deadline: '', status: 'open', notes: '' })
    }
  }, [transfer, open])

  function set(k, v) { setForm(f => ({ ...f, [k]: v })) }

  // Auto-set return deadline to 60 days from transfer date
  function handleTransferDateChange(date) {
    ...
    <Modal open={open} onClose={onClose} title={transfer ? 'Köçürməni redaktə et' : 'Yeni daxili köçürmə'}>
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-[#555] mb-1">Haradan (layihə) *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[#555] mb-1">Haraya (layihə) *</label>
          ...
          <div>
            ...
            <input type="date" value={form.transfer_date} onChange={e => handleTransferDateChange(e.target.value)}
              className="w-full px-3 py-2 border border-[#e8e8e4] rounded-lg text-sm focus:outline-none focus:border-[#0f172a]" />
          </div>
          <div>
...
export default function DaxiliKocurmelePage() {
  ...
  async function handleSave(form) {
    if (!form.from_project_id || !form.to_project_id || !form.amount) {
      addToast('Layihə və məbləğ daxil edin', 'error'); return
    }
    const data = {
  ...
  async function markReturned(t) {
    await supabase.from('internal_transfers').update({
      status: 'returned', returned: true,
      returned_date: new Date().toISOString().split('T')[0]
    }).eq('id', t.id)
    addToast('Qaytarıldı olaraq işarələndi', 'success')
    await loadData()
  }
  ...
  const getDaysLeft = (deadline) => {
    ...
    return Math.floor((new Date(deadline) - today) / 86400000)
  }
  ...
  return (
    <div className="p-6 fade-in">
      <PageHeader
        title="Daxili Köçürmələr"
        subtitle="Layihələrarası borc izlənməsi · 60 gün qaytarma qaydası"
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        ...
        <StatCard label="Açıq məbləğ" value={fmt(totalOpen)} variant={totalOpen > 0 ? 'danger' : 'default'} />
      ...
      {transfers.length === 0 ? (
        <EmptyState icon={IconArrowsExchange} title="Hələ daxili köçürmə yoxdur"
          description="Layihələrarası pul köçürmələrini izləyin"
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#e8e8e4]">
                  <th className="text-left px-4 py-3 font-medium text-[#888]">Haradan</th>
                  <th className="text-left px-4 py-3 font-medium text-[#888]">Haraya</th>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-[#888]">Qaytarma</th>
              ...
              <tbody>
                {transfers.map(t => {
                  const daysLeft = getDaysLeft(t.return_deadline)
                  ...
                  return (
                    <tr key={t.id} className={`border-b border-[#f5f5f0] hover:bg-[#fafaf8] ${isOverdue ? 'bg-red-50/30' : isUrgent ? 'bg-yellow-50/30' : ''}`}>
                      <td className="px-4 py-3 font-medium text-[#0f172a]">
                        {getProject(t.from_project_id)?.name || '—'}
                      </td>
                      <td className="px-4 py-3 text-[#555]">
                        {getProject(t.to_project_id)?.name || '—'}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {t.return_deadline ? (
                          <span className={`font-medium ${isOverdue ? 'text-red-600' : isUrgent ? 'text-yellow-600' : 'text-[#555]'}`}>
                            {new Date(t.return_deadline).toLocaleDateString('az-AZ')}
                            {isOverdue && <span className="ml-1 text-[10px]">({Math.abs(daysLeft)}g keçmiş)</span>}
                            {isUrgent && !isOverdue && <span className="ml-1 text-[10px]">({daysLeft}g qalıb)</span>}
                          </span>
                        ) : '—'}
                      </td>
                      <td className="px-4 py-3"><Badge variant={st?.color} size="sm">{st?.label}</Badge></td>
                      <td className="px-4 py-3 text-right font-bold text-[#0f172a]">{fmt(t.amount)}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          {t.status === 'open' && (
                            <button onClick={() => markReturned(t)} className="text-[#aaa] hover:text-green-600 p-1" title="Qaytarıldı">
                              <IconCheck size={12} />
                            </button>
                          )}
              ...
              <tfoot>
                <tr className="bg-[#f5f5f0]">
                  <td colSpan={6} className="px-4 py-2 text-xs font-medium text-[#555]">Açıq köçürmələr cəmi</td>
                  <td className="px-4 py-2 text-right text-xs font-bold text-[#0f172a]">{fmt(totalOpen)}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
