...
function TaskForm({ open, onClose, onSave, task, projects, members }) {
  const [form, setForm] = useState({ title: '', description: '', project_id: '', assignee_id: '', status: 'not_started', priority: 'medium', due_date: '' })

  useEffect(() => {
    if (task) {
      setForm({ title: task.title || '', description: task.description || '', project_id: task.project_id || '', assignee_id: task.assignee_id || '', status: task.status || 'not_started', priority: task.priority || 'medium', due_date: task.due_date || '' })
    } else {
...
export default function TapshiriqlarPage() {
  ...
  async function handleSave(form) {
    const data = { title: form.title, description: form.description, project_id: form.project_id || null, assignee_id: form.assignee_id || null, status: form.status, priority: form.priority, due_date: form.due_date || null }
    if (editTask) {
  ...
  const grouped = projects.reduce((acc, p) => {
    ...
    if (pts.length > 0) acc[p.id] = { name: p.name, tasks: pts }
    return acc
  }, {})
  ...
  return (
    <div className="p-6 fade-in">
      <PageHeader
        ...
        action={<Button onClick={() => { setEditTask(null); setModalOpen(true) }} size="sm"><IconPlus size={14} /> Yeni tapşırıq</Button>}
      />
      ...
      <div className="flex gap-1 mb-4 border-b border-[#e8e8e4]">
        {[{ key: 'all', label: 'Hamısı' }, ...STATUSES].map(s => (
          <button key={s.key} onClick={() => setFilter(s.key)}
            ...
            {s.label}
            <span className="ml-1 text-[10px] text-[#aaa]">{s.key === 'all' ? tasks.length : tasks.filter(t => t.status === s.key).length}</span>
          </button>
        ))}
      </div>

      {tasks.length === 0 ? (
        <EmptyState icon={IconCheckbox} title="Hələ tapşırıq yoxdur" description="İlk tapşırığı əlavə edin"
        ...
        <div className="space-y-4">
          {Object.values(grouped).map(group => (
            <Card key={group.name}>
              ...
              <div>
                {group.tasks.filter(t => filter === 'all' || t.status === filter).map(task => (
                  <TaskRow key={task.id} task={task} onEdit={t => { setEditTask(t); setModalOpen(true) }} onDelete={setDeleteTask} onToggle={toggleStatus} />
                ))}
              </div>
            </Card>
          ))}
          {noProject.filter(t => filter === 'all' || t.status === filter).length > 0 && (
            <Card>
              <div className="px-4 py-3 border-b border-[#f0f0ec]">
                <span className="text-xs font-bold text-[#888]">Layihəsiz tapşırıqlar</span>
              </div>
              {noProject.filter(t => filter === 'all' || t.status === filter).map(task => (
                <TaskRow key={task.id} task={task} onEdit={t => { setEditTask(t); setModalOpen(true) }} onDelete={setDeleteTask} onToggle={toggleStatus} />
              ))}
            </Card>
          )}
      ...
      <TaskForm open={modalOpen} onClose={() => { setModalOpen(false); setEditTask(null) }} onSave={handleSave} task={editTask} projects={projects} members={members} />
      <ConfirmDialog open={!!deleteTask} title="Tapşırığı sil" message={`"${deleteTask?.title}" tapşırığını silmək istədiyinizə əminsiniz?`} onConfirm={handleDelete} onCancel={() => setDeleteTask(null)} danger />
    </div>
  )
}
